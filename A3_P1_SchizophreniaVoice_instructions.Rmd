...
```{r}
library(pacman)
p_load(tidyverse, stringr, effsize, lme4)
...
merged_data %>% group_by(Gender,Diagnosis.x) %>% summarise(Participants = length(unique(ID)),mean_age=mean(Age, na.rm = T),meanVIQ=mean(VerbalIQ, na.rm = T),SD_VIQ=sd(VerbalIQ, na.rm = T),meansans=mean(as.numeric(SANS), na.rm = T),meansaps=mean(as.numeric(SAPS), na.rm = T))

merged_data %>% group_by(Study.x,Gender,Diagnosis.x) %>% summarise(Participants = length(unique(ID)), mean_age=mean(Age, na.rm = T),meanVIQ=mean(VerbalIQ, na.rm = T),SD_VIQ=sd(VerbalIQ, na.rm = T),meansans=mean(as.numeric(SANS), na.rm = T),meansaps=mean(as.numeric(SAPS), na.rm = T))
```
